...
**Область деятельности**: организация управление отоплением в отдельном доме. 
...
**Цель**: компания планирует разворачивание управления на большое количество разных домов в различных георграфических регионах, так же должна быть увеличена номенклатура предоставляемых сервисов управления домом, количество и их тип может варироваться. В домах, которыми планируется управлять, ожидается наличие специальныъх блоки датчиков и реле. На текущий момент 50% домов уже оборудованы требуемыми блоками управления.
...
-  Кроме получения информацмии о текущем состоянии непосредственно конкретного контроллера, система реализует возможность получения информации о состоянии всех контроллеров указанного помещения (по его наименованию), возвращается набор описателей с данными: 
...
- Система обрабатывает некорректное указание помещения - либо пустое, либо нзарегистрированное. В случае корректности данных возвращается набор значений.
